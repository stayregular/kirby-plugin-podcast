...
function curl_get_file_size( $url ) {
  ...
  curl_setopt( $curl, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:47.0) Gecko/20100101 Firefox/47.0 Mozilla/5.0 (Macintosh; Intel Mac OS X x.y; rv:42.0) Gecko/20100101 Firefox/42.0' );
...
// Update the fields when new page is created or updated

kirby()->hook(['panel.page.create', 'panel.page.update'], function($page) {
  //check if MP3 field
  if($page->content()->data['mp3']) {
    $mp3 = $page->mp3()->value();

    // Get filesize of MP3
    $filesize = curl_get_file_size($page->mp3()->value());

    // If filesize differs, save new MP3 data
    if($filesize != $page->filesize()->value()) {


      $mp3file = new MP3File($page->mp3()->value());//http://www.npr.org/rss/podcast.php?id=510282
